...
```python
users.value_counts('age',ascending=True).head(1)
```
...
```python
drinks.groupby('continent').mean('beer_servings').sort_values('beer_servings', ascending=False).head(1)
```
...
```python
drinks.groupby('continent').sum('wine_servings')['wine_servings']
```
